...
syn match cssUIProp contained "\<nav-\(index\|up\|right\|down\)\>" containedin=ALL
syn keyword cssUIProp contained resize outline
" Other modules
syn match cssCommonAttr contained "\(width\|span\|rule\|gap\|fill\|count\)\>" containedin=ALL
syn match cssCommonAttr contained "\(color\|width\|style\)\>" containedin=ALL
syn match cssCommonAttr contained "\(after\|before\)\>" containedin=ALL
...
syn match cssTextAttr contained "\<text-shadow\|-moz-\|-webkit-\|-o-\|transition\|text-overflow\|word-wrap\|left\|right\>" containedin=ALL
